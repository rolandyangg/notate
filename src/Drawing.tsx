...
const DrawingCanvas = () => {
  ...
  const [activeTextBox, setActiveTextBox] = useState<null | { x: number; y: number }>(null);
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (tool === "text") {
      if (!activeTextBox || activeTextBox.x !== x || activeTextBox.y !== y) {
        setActiveTextBox({ x, y });
        ...
        <input
          ...
          onBlur={(e) => {
            ...
            if (text !== "") {
              ...
              ctx.fillText(text, activeTextBox.x, activeTextBox.y + brushSize * 4);
              saveState();
            }
